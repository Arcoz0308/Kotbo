...
<div class="relative" {id}>
  ...
  <div
    ...
    class="flex flex-wrap gap-1.5 min-h-11 w-full bg-surface-container-high/40 border border-outline-variant/10 rounded-2xl px-3 py-2 cursor-text transition-all {open ? 'ring-2 ring-primary/30' : ''} {disabled ? 'opacity-50 cursor-not-allowed' : ''}"
  ...
  {#if open && !disabled}
    <div
      id="{id}-listbox"
      role="listbox"
      class="absolute left-0 right-0 mt-2 z-30 rounded-2xl border border-outline-variant/20 bg-surface-container-high shadow-2xl max-h-52 overflow-y-auto"
    >
