...
{#if open}
  ...
  <div class="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-6" onclick={onClose}>
    ...
    <div 
      class="relative w-full max-w-5xl h-[95vh] flex flex-col premium-card rounded-[2.5rem] overflow-hidden animate-in fade-in zoom-in-95 duration-300"
      onclick={(e) => e.stopPropagation()}
    >
      <!-- Header -->
      <div class="p-6 md:p-8 flex items-start justify-between border-b border-outline-variant/10 shrink-0">
        <div class="flex items-center gap-4">
          <div class="p-3 rounded-2xl {iconBgClass} {iconColorClass}">
            <Papicon {icon} size={24} />
          </div>
          <div>
            <h3 class="text-xl md:text-2xl font-black text-on-surface">{title}</h3>
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 space-y-8">
        ...
        <!-- Toolbar: Search & Sort -->
        <div class="flex flex-col md:flex-row gap-4 p-4 rounded-3xl bg-surface-container-high/20 border border-outline-variant/5">
          <div class="flex-1 relative">
            ...
            <input 
              ...
              class="w-full bg-surface-container-high/40 text-on-surface text-sm font-bold rounded-2xl pl-12 pr-4 py-3 border border-outline-variant/5 focus:border-primary/50 focus:outline-none transition-colors placeholder:text-on-surface-variant/40"
            />
          </div>
          <div class="flex gap-2">
            <select 
              bind:value={sortKey}
              class="bg-surface-container-high/40 text-on-surface text-sm font-bold rounded-2xl px-4 py-3 border border-outline-variant/5 focus:border-primary/50 focus:outline-none transition-colors appearance-none cursor-pointer"
            >
            ...
            <button 
              ...
              class="px-4 py-3 rounded-2xl bg-surface-container-high/40 hover:bg-surface-container-high text-on-surface transition-colors flex items-center justify-center gap-2 border border-outline-variant/5"
            >
        ...
        <div class="space-y-3">
          {#each paginatedData as item, i}
            ...
            {#if type === 'messages' || type === 'voice'}
              ...
              >
                <div class="flex items-center gap-4">
                  <div class="w-8 text-center text-xs font-black text-on-surface-variant/40">{globalIndex}</div>
                  <div class="relative">
                  ...
                  <p class="text-base font-black text-on-surface">@{item.name || item.username}</p>
                </div>
                <div class="flex items-center gap-6">
                  <div class="text-right">
                    <p class="text-[10px] font-black {type === 'messages' ? 'text-primary' : 'text-emerald-500'} uppercase tracking-widest">{type === 'messages' ? 'Messages' : 'Minutes'}</p>
                    <p class="text-base font-black text-on-surface">{(type === 'messages' ? item.messageCount : Math.round(item.voiceTimeSeconds / 60)).toLocaleString('fr-FR')}</p>
                  </div>
            ...
            {:else if type === 'channels'}
              <div class="w-full flex items-center justify-between p-4 rounded-2xl bg-surface-container-high/20 border border-outline-variant/5">
                <div class="flex items-center gap-4">
                  <div class="w-8 text-center text-xs font-black text-on-surface-variant/40">{globalIndex}</div>
                  <div class="bg-secondary/10 p-3 rounded-xl text-secondary">
                    <Papicon icon="Hash" size={20} />
                  </div>
                  <p class="text-base font-black text-on-surface">#{item.channelName || item.name || item.channelId}</p>
                </div>
                <div class="text-right">
                  <p class="text-[10px] font-black text-secondary uppercase tracking-widest">Volume</p>
                  <p class="text-base font-black text-on-surface">{(item.messagesCount || item.count).toLocaleString('fr-FR')}</p>
                </div>
              </div>
            ...
            {:else if type === 'moderators'}
              <button 
                ...
                class="w-full flex items-center justify-between p-4 rounded-2xl bg-surface-container-high/20 border border-outline-variant/5 hover:bg-surface-container-high/60 transition-all text-left"
              >
                <div class="flex items-center gap-4">
                  <div class="w-8 text-center text-xs font-black text-on-surface-variant/40">{globalIndex}</div>
                  <img src={getAvatar(item.avatarUrl)} alt="" class="w-10 h-10 rounded-xl object-cover" />
                  <p class="text-base font-black text-on-surface">@{item.moderatorTag}</p>
                </div>
                <div class="text-right">
                  <p class="text-[10px] font-black text-primary uppercase tracking-widest">Actions</p>
                  <p class="text-base font-black text-on-surface">{item.count}</p>
                </div>
              </button>
            ...
            {:else if type === 'sanctioned'}
              ...
              >
                <div class="flex items-center gap-4">
                  <div class="w-8 text-center text-xs font-black text-on-surface-variant/40">{globalIndex}</div>
                  <img src={getAvatar(item.avatarUrl)} alt="" class="w-10 h-10 rounded-xl object-cover" />
                  <p class="text-base font-black text-on-surface">@{item.targetTag}</p>
                </div>
                <div class="text-right">
                  <p class="text-[10px] font-black text-rose-500 uppercase tracking-widest">Sanctions</p>
                  <p class="text-base font-black text-on-surface">{item.count}</p>
                </div>
              </button>
            ...
            {:else if type === 'staff'}
              ...
              >
                <div class="flex items-center justify-center w-8 h-8 rounded-full font-black text-xs {globalIndex <= 3 ? 'bg-amber-500/10 text-amber-500' : 'bg-on-surface/5 text-on-surface-variant/30 shrink-0'}">
                  {globalIndex}
                </div>
                ...
                <div class="flex-1 min-w-0">
                  <span class="text-base font-black text-on-surface block truncate">{item.name}</span>
                ...
                <div class="text-right pl-4 border-l border-outline-variant/10 shrink-0">
                  ...
                  <p class="text-lg font-black text-primary">{fmt?.(item.score)}</p>
                </div>
              </button>
            ...
            {:else if type === 'recent_sanctions'}
              <div class="flex flex-col md:flex-row md:items-center justify-between p-5 rounded-3xl bg-surface-container-high/30 border border-outline-variant/10 hover:bg-surface-container-high/50 transition-all group gap-4">
                <div class="flex items-center gap-4">
                  <div class="w-8 text-center text-xs font-black text-on-surface-variant/40 hidden md:block">{globalIndex}</div>
                  <button 
                    onclick={() => onOpenMember?.(item.targetUserId, item.targetTag)}
                  ...
                  <div>
                    <div class="flex items-center gap-2">
                      <p class="text-sm font-black text-on-surface">@{item.targetTag}</p>
                      {#if getSanctionColor}
                        <span class="px-2 py-0.5 rounded-lg text-[9px] font-black tracking-widest uppercase" style="background: {getSanctionColor(item.type)}20; color: {getSanctionColor(item.type)}">{item.type}</span>
                      {/if}
                    </div>
