...
<div class="space-y-8">
  <!-- Header -->
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 premium-card p-6 md:p-8 rounded-[2.5rem]">
    <div class="flex items-center gap-4">
      <div class="bg-primary/10 p-4 rounded-3xl text-primary shadow-inner">
        <Papicon icon="calendar" size={28} />
      </div>
      <div>
        <h3 class="text-2xl font-black text-on-surface tracking-tight">Comparaison de Périodes</h3>
    ...
    <div class="flex flex-col gap-3">
      ...
      <div class="flex items-center gap-2 bg-surface-container-high/40 p-1.5 rounded-2xl border border-outline-variant/10 self-start">
        <button
          onclick={() => setMode('week')}
          class="flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 {selectedMode === 'week' ? 'bg-primary text-on-primary shadow-lg' : 'text-on-surface-variant/60 hover:text-on-surface'}"
        >
        ...
        <button
          ...
          class="flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 {selectedMode === 'month' ? 'bg-primary text-on-primary shadow-lg' : 'text-on-surface-variant/60 hover:text-on-surface'}"
        >
      ...
      <div class="flex flex-wrap gap-1.5">
        {#each currentOptions as opt}
          <button
            ...
            class="px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all duration-200 border {selectedOffset === opt.offset ? 'bg-primary/10 text-primary border-primary/30 shadow-inner' : 'text-on-surface-variant/50 border-outline-variant/10 hover:bg-surface-container-high hover:text-on-surface'}"
          >
  ...
  <div class="flex flex-wrap items-center gap-3">
    <div class="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-400/10 border border-slate-400/15 text-[10px] font-black uppercase text-slate-400">
      <div class="w-2 h-2 rounded-full bg-slate-400/50"></div>
      {previousPeriodLabel}
    </div>
    <div class="text-on-surface-variant/30 text-xs font-bold">→</div>
    <div class="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 border border-primary/20 text-[10px] font-black uppercase text-primary">
  ...
  {#if data}
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
      {#each metrics as metric}
        ...
        <div class="premium-card p-6 rounded-[2rem] border border-outline-variant/10 hover:border-primary/20 transition-all duration-500 group relative overflow-hidden">
          <!-- Glow -->
          <div class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 rounded-[2rem]" style="background: radial-gradient(ellipse at top left, {metric.accentLight} 0%, transparent 60%)"></div>
          
          <!-- Header -->
          <div class="relative flex items-start justify-between mb-5">
            <div class="flex items-center gap-3">
              <div class="p-2.5 rounded-xl border border-outline-variant/10 bg-surface-container-low group-hover:scale-110 transition-transform duration-500" style="color: {metric.color}">
                <Papicon icon={metric.icon} size={20} />
              </div>
              <p class="text-[10px] font-black text-on-surface-variant/50 uppercase tracking-[0.2em]">{metric.label}</p>
            </div>
            <div class="px-2.5 py-1 rounded-xl flex items-center gap-1 border text-xs font-black {getChangeColor(metric.change)}">
          ...
          <div class="relative space-y-3 mb-5">
            ...
            <div class="space-y-1">
              <div class="flex items-center justify-between text-[9px] font-black uppercase tracking-widest">
                ...
                <span class="text-on-surface font-black text-sm">
          ...
          <div class="relative pt-4 border-t border-outline-variant/10 flex items-center justify-between">
            <span class="text-[9px] font-black uppercase tracking-widest text-on-surface-variant/30">Variation absolue</span>
            <span class="text-xs font-black {pct >= 0 ? 'text-emerald-400' : 'text-red-400'}">
      ...
      <!-- Summary Card -->
      <div class="premium-card p-8 rounded-[2rem] bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-primary/10 flex flex-col justify-center">
        <div class="space-y-4">
          ...
          <h4 class="text-2xl font-black text-on-surface leading-tight">
