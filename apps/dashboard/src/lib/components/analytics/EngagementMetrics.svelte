<script lang="ts">
  ...
  const channelsChartData = $derived({
    labels: topChannels.slice(0, 5).map(c => `#${c.channelName || c.name}`),
    datasets: [{
      label: 'Volume',
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
  ...
  <div class="premium-card p-8 rounded-[2.5rem] space-y-8 flex flex-col">
    ...
    <div class="space-y-3 flex-grow overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
      {#each (showAll ? topMembers : topMembers.slice(0, 3)) as member}
        <button 
    ...
    <div class="grid grid-cols-2 gap-4">
       {#each topChannels.slice(0, 4) as channel}
          <div class="p-4 rounded-2xl bg-secondary/5 border border-secondary/10 hover:border-secondary/30 transition-colors">
             <p class="text-[9px] font-black uppercase tracking-widest text-secondary/60 mb-1">#{channel.channelName || channel.name}</p>
