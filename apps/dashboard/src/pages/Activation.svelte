      ...
      <div class="flex items-center gap-4">
        {#if authStore.user}
          <div class="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-2 rounded-2xl backdrop-blur-md">
            ...
            <button onclick={logout} class="text-[9px] font-black text-rose-500 uppercase tracking-widest hover:underline">Déconnexion</button>
          </div>
        {/if}
      </div>
    </nav>
  ...
  <main class="relative z-10 flex-grow flex items-center justify-center px-6 py-12">
    <div class="w-full max-w-[580px] animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      <div class="relative group">
        ...
        <div class="relative bg-slate-900/40 backdrop-blur-3xl rounded-[3rem] border border-white/10 p-10 md:p-14 shadow-2xl overflow-hidden">
          ...
          <div class="flex flex-col items-center text-center mb-10">
            <div class="relative mb-6">
              ...
              {#if guildIconUrl}
                <img src={guildIconUrl} alt="Server Logo" class="relative w-24 h-24 rounded-3xl object-cover shadow-xl border border-white/10" />
              {:else}
                <div class="relative w-24 h-24 rounded-3xl bg-primary/10 flex items-center justify-center border border-primary/20">
            ...
            <div class="space-y-4">
              <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20">
                 ...
                 <span class="text-[9px] font-black uppercase tracking-[0.2em] text-amber-500">Sécurité requise</span>
              </div>
              <h1 class="font-headline text-3xl font-black tracking-tighter text-white leading-tight">
          ...
          <div class="space-y-6">
            {#if errorMessage}
              <div class="p-4 rounded-[1.5rem] bg-rose-500/10 border border-rose-500/20 flex items-center gap-4 animate-in fade-in duration-500">
            ...
            <div class="space-y-2">
              <label for="activation-code" class="text-[10px] font-black text-slate-500 uppercase tracking-widest block ml-1">Clé d'activation globale</label>
              <div class="relative">
                ...
                <input 
                  ...
                  disabled={loading}
                  class="w-full pl-12 pr-4 py-4 rounded-[1.5rem] bg-slate-950/50 border border-white/10 text-white font-mono placeholder:text-slate-600 focus:outline-hidden focus:border-primary/50 transition-colors uppercase"
                />
              </div>
            </div>
            ...
            >
              <div class="absolute inset-0 bg-primary rounded-[1.5rem] transition-all duration-500 group-hover/btn:scale-105 group-hover/btn:shadow-[0_0_40px_rgba(51,69,87,0.4)] disabled:opacity-50"></div>
            ...
            {#if authStore.guilds.length > 1}
              <div class="pt-6 border-t border-white/5 space-y-3">
                <label for="guild-select" class="text-[10px] font-black text-slate-500 uppercase tracking-widest block text-center">Accéder à un autre serveur</label>
                <div class="relative">
                  <select 
                    id="guild-select"
                    value={authStore.selectedGuildId}
                    onchange={(e) => handleSelectGuild((e.target as HTMLSelectElement).value)}
                    class="w-full px-4 py-3 rounded-2xl bg-white/5 border border-white/10 text-xs font-bold text-slate-300 focus:outline-hidden focus:border-primary/30 transition-colors cursor-pointer appearance-none text-center"
                  >
