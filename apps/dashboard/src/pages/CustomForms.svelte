  ...
  {#snippet actions()}
    <div class="flex items-center gap-3">
      ...
      <button 
        ...
        class="px-5 py-2.5 bg-primary text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-primary/20 hover:scale-105 transition-transform flex items-center gap-2"
      >
  ...
  <div class="space-y-8">
    ...
    {:else if forms.length === 0}
      <div class="flex flex-col items-center justify-center py-32 text-on-surface-variant/30 border-2 border-dashed border-outline-variant/10 rounded-[4rem] bg-surface-container-low/20">
        <div class="w-24 h-24 rounded-[3rem] bg-surface-container flex items-center justify-center mb-6 shadow-inner text-purple-400">
          <Papicon icon="description" size={48} />
        </div>
        <h3 class="text-2xl font-black tracking-tight text-on-surface/50 font-sans">Aucun formulaire</h3>
    ...
    {:else}
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {#each forms as form (form.id)}
          <div class="bg-surface-container-low/40 border border-outline-variant/10 rounded-4xl p-6 hover:bg-surface-container-low transition-all group relative overflow-hidden flex flex-col justify-between">
            <div>
              <div class="flex items-start justify-between mb-4">
                <div class="flex-1 min-w-0">
                  <h3 class="text-lg font-black text-on-surface truncate font-sans">{form.name}</h3>
                  {#if form.description}
            ...
            <div class="flex gap-2 w-full pt-4 border-t border-outline-variant/10">
              <button
                ...
                class="flex-1 px-3 py-2.5 rounded-xl bg-primary/10 text-primary text-xs font-black uppercase tracking-wider hover:bg-primary/20 transition-all flex items-center justify-center gap-1.5"
              ...
              <button
                ...
                class="flex-1 px-3 py-2.5 rounded-xl bg-surface-container text-on-surface-variant text-xs font-black uppercase tracking-wider hover:bg-surface-container-high transition-all flex items-center justify-center gap-1.5"
              ...
              <button
                ...
                class="px-3 py-2.5 rounded-xl bg-rose-500/10 text-rose-500 text-xs font-black uppercase tracking-wider hover:bg-rose-500/20 transition-all"
                title="Supprimer"
              >
...
{#if showCreateModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
    <div class="bg-surface border border-outline-variant/30 rounded-[2.5rem] w-full max-w-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
      <div class="p-8 border-b border-outline-variant/20 flex items-center justify-between">
        <div>
          <h3 class="text-xl font-black text-on-surface font-sans">Créer un Formulaire Autonome</h3>
      ...
      <div class="p-8 space-y-6">
        <div>
          <label for="form-name" class="block text-xs font-black uppercase tracking-widest text-on-surface-variant/60 mb-2 font-sans">Nom du formulaire</label>
          <FormInput 
            id="form-name"
            type="text" 
        ...
        <div>
          <label for="form-description" class="block text-xs font-black uppercase tracking-widest text-on-surface-variant/60 mb-2 font-sans">Description (optionnelle)</label>
          <textarea 
            id="form-description"
            bind:value={newFormDescription}
            placeholder="Description du formulaire..."
            class="w-full bg-surface-container rounded-2xl p-4 text-sm text-on-surface placeholder:text-on-surface-variant/60 focus:outline-hidden border border-outline-variant/10 focus:border-primary/50 transition-all resize-none h-24 font-sans"
          ></textarea>
        </div>
      </div>
      ...
      <div class="p-8 bg-surface-container-low border-t border-outline-variant/20 flex gap-4">
        ...
        <button 
          ...
          class="flex-1 py-3.5 rounded-xl font-black bg-primary text-on-primary hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg shadow-primary/20 disabled:opacity-50 disabled:scale-100 font-sans"
        >
