...
export async function createBackup(guild: Guild, options: BackupOptions) {
  ...
  if (options.includeStickers) {
    ...
    try {
      ...
      const stickers = await Promise.race([
        ...
        new Promise<any>((_, reject) => setTimeout(() => reject(new Error('Timeout fetching stickers')), 5000))
      ]);
      backupData.stickers = stickers.map((sticker: any) => ({
        id: sticker.id,
        name: sticker.name,
  ...
  if (options.includeMessages) {
    ...
    for (const [channelId, channel] of textChannels) {
      try {
        const fetchedMessages = await (channel as any).messages.fetch({ limit: 50 });
  ...
  try {
    const backup = await (prisma as any).serverBackup.create({
      data: {
        guildId: guild.id,
        name: options.name,
