...
export async function createCustomForm(
  guildId: string,
  data: { name: string; description?: string; structure: CustomFormStructure; isRecruitment?: boolean }
) {
...
export async function updateCustomForm(
  formId: string,
  guildId: string,
  data: { name?: string; description?: string; structure?: CustomFormStructure; isActive?: boolean; isRecruitment?: boolean }
) {
...
export async function submitCustomForm(
  ...
  // @ts-expect-error - Prisma client needs to be regenerated by user to recognize customFormSubmission
  ...
  try {
    ...
    if (form?.isRecruitment) {
      // @ts-expect-error - Prisma client needs to be regenerated by user to recognize customFormId
...
export async function handleFormModalSubmit(
  ...
  if (eventId) {
    // @ts-expect-error - Prisma client needs to be regenerated by user to recognize customEventRegistration
